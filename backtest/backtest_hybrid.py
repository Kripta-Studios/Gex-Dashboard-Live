...
class TradeSimulator:
    ...
    def simulate(self, df: pd.DataFrame, predictions: np.ndarray, probabilities: np.ndarray, time_predictions: np.ndarray = None) -> pd.DataFrame:
        ...
        balance = 10000.0    # Base balance for equity recovery tracking
        ...
        for idx, row in df_work.iterrows():
            ...
            # Apply contract limits and drawdown penalties
            current_contracts = float(self.position_size)
            current_contracts = min(current_contracts, 1000.0) # Max 1000 spot contracts
            if in_drawdown_mode:
                current_contracts = max(1.0, current_contracts * 0.5)
            
            pnl_dollars = 0.0
            if direction == "LONG":
                pnl = (exit_price - entry_price)
                pnl_dollars = pnl * multiplier * current_contracts

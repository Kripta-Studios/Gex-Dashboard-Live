...
La fuente wall/IB indicada por el usuario se auditó por separado porque no estaba en esas 154 features. El join entre labels ask→bid y las features live fue exacto en 86.729 filas 202208–202512, con spot idéntico y sin 2026. La regla mecánica magnet/rejection/acceptance V1 perdió en los tres tickers (PF `0,865/0,832/0,899`). El control de mera proximidad también perdió. La atribución encontró que la definición de rejection no exigía haber cruzado el nivel; solo 29–37% eran rechazos reales. V1r1 corrigió ese único factor y mejoró SPXW/QQQ/SPY a PF `0,910/0,845/1,008`; SPY quedó ligeramente positivo, pero ninguno cumple el contrato. No se tunearán horas/subfamilias sobre este output. La señal parcial de acceptance CALL y resistance-rejection PUT no tiene volumen/estabilidad suficiente. Falta información causal que el feature contract actual no materializa: magnitud, concentración, persistencia y migración de cada wall, además del strike max/min de delta exposure. Siguiente trabajo: construir ese estado desde ThetaData con paridad live y comprobar primero hit/reject/break del subyacente a 30–180m antes de mapearlo a opciones.

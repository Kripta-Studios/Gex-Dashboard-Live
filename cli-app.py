...
async def main(ticker, expiration, greek_filter):
    ...
    if greek_filter != None:
        a = await get_options_data(
            ticker.upper().replace(" ", ""),
            expiration.lower().replace(" ", ""),
            greek_filter.lower().replace(" ", ""),
        )
    else:
        a = await get_options_data(
            ticker.upper().replace(" ", ""),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "expiration", type=str, help="Expiration in dte, or weekly, opex, monthly, all"
    )
    parser.add_argument(
        "greek",
        type=str,
        nargs="?",
        default=None,
        help="Greek to plot: vanna, charm, gamma o delta",
    )

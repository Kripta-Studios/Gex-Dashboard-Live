...
def main() -> int:
    ...
    feature_cols = [c for c in xinput.columns if str(c).startswith("xjepa_")]
    if not feature_cols:
        raise RuntimeError(f"No xjepa_* columns found in {xinput_path}")

    x = xinput[["ticker", "date", "time", *feature_cols]].copy()
    x["_xinput_ticker"] = x["ticker"].astype(str).str.upper()
    x["_xinput_date"] = x["date"].map(normalize_date)
    x["_xinput_time"] = x["time"].map(normalize_time)
    x = (
        x.drop(columns=["ticker", "date", "time"])

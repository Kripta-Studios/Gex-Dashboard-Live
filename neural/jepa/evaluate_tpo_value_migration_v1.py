"""Nested executable evaluation for H-TPOVALUE1.

Development is fixed to 2023-04..2023-12.  Frozen mode requires the exact
outcome-free view, a complete committed code/protocol closure and the single
predeclared 2024-2025 output target.  This runner never promotes a base
economic pass while exact live TPO parity and execution stress remain pending.
"""
...
VIEW = ROOT / "tmp/existing_data_edge_sprint_v1/tpo_value_migration_v1/modeling_view.parquet"
VIEW_MANIFEST = ROOT / "tmp/existing_data_edge_sprint_v1/tpo_value_migration_v1/manifest.json"
...
FROZEN_CODE_CLOSURE = (
    "neural/jepa/build_tpo_value_migration_view_v1.py",
    "neural/jepa/evaluate_tpo_value_migration_v1.py",
    ...
    "neural/jepa/freeze_tpo_value_migration_v1.py",
)
FROZEN_PROTOCOL_CLOSURE = (
    "research_papers/JEPA/H_TPOVALUE1_EXECUTABLE_PREDECLARATION.md",
    "research_papers/JEPA/CROSS_MARKET_TRANSMISSION_V1R1_HALF_DAY_REPAIR.md",
)
FROZEN_OUTER_OUTPUT = (
    "research_papers/JEPA/results/_diagnostics/"
    "h_tpovalue1_executable_v1_202401_202512"
)
MAX_TOP5_TRADE_GROSS_PROFIT_SHARE = 0.20
MAX_TOP5_DAY_GROSS_PROFIT_SHARE = 0.30
FOLD_CHECKPOINT_SCHEMA = "h_tpovalue1_economic_fold_checkpoint_v1"
RUN_CHECKPOINT_SCHEMA = "h_tpovalue1_economic_run_checkpoint_v1"
...
def runner_protocol() -> dict[str, Any]:
    return {
        "schema": "tpo_value_migration_runner_protocol_v1",
        ...
        "checkpoint": {
            "schema": FOLD_CHECKPOINT_SCHEMA,
            "granularity": "outer_month_ticker_arm",
        ...
        "concentration_gates": {
            "pooled_and_each_ticker_top5_trade_gross_profit_share_max": (
                MAX_TOP5_TRADE_GROSS_PROFIT_SHARE
            ),
            "pooled_and_each_ticker_top5_day_gross_profit_share_max": (
...
def _verify_view(view_path: Path, manifest_path: Path) -> dict[str, Any]:
    ...
    if mismatches:
        raise AssertionError(f"TPO value view contract failed: {mismatches}")
    if manifest.get("view_sha256") != sha256_file(view_path):
        raise AssertionError("TPO value view hash mismatch")
    if manifest.get("date_max") != "20251231":
        raise AssertionError("TPO value view date boundary changed")
    if manifest.get("excluded_early_close_dates") != list(EARLY_CLOSE_DATES):
        raise AssertionError("early-close exclusion changed")
    parquet = pq.ParquetFile(view_path)
    if parquet.metadata.num_rows != ELIGIBLE_ROWS_V1R1:
        raise AssertionError("view physical row count differs from the frozen census")
    expected_x0 = list(PAIRWISE_E0_FEATURES)
    expected_x1 = [*expected_x0, *TPO_FEATURES]
    ...
    if _canonical_sha(list(TPO_FEATURES)) != EXPECTED_TPO_SHA256:
        raise AssertionError("active TPO block hash changed")
    for arm, count in (("X0", 30), ("X1", 66)):
...
def _verify_frozen_manifest(path: Path, view_manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    if (
        frozen.get("schema") != "h_tpovalue1_executable_v1_frozen_runner"
        or frozen.get("status") != "PREEXECUTION_FROZEN"
    ):
    ...
    active_hashes = {
        relative: sha256_file(ROOT / relative) for relative in FROZEN_CODE_CLOSURE
    }
    if active_hashes != frozen.get("code_hashes"):
        raise AssertionError("active runner code differs from freeze")
    active_protocol_hashes = {
        relative: sha256_file(ROOT / relative) for relative in FROZEN_PROTOCOL_CLOSURE
    }
    if active_protocol_hashes != frozen.get("protocol_hashes"):
        raise AssertionError("active protocol documents differ from freeze")
    ...
    *,
    ...
    if len(features) != len(outcomes) or len(missing_view) or len(missing_master):
        raise AssertionError(
            "feature/outcome bidirectional key parity failed: "
...
def economic_metrics(trades: pd.DataFrame) -> dict[str, float | int]:
    ...
    ordered = trades.copy()
    ordered["exit_sort_minute"] = pd.to_numeric(ordered["minute"], errors="raise") + pd.to_numeric(
        ordered["exit_minutes"], errors="raise"
    )
    ...
    realized_by_exit = ordered.groupby(
        ["trade_date", "exit_sort_minute"], sort=True, observed=True
    )["realized_return"].sum()
        ...
        row = {
            **config,
            ...
            "inner_month_metrics": json.dumps(
                dict(zip(inner_months, metrics)), sort_keys=True, allow_nan=True
            ),
        }
        rows.append(row)
        if row["passed"]:
            ...
            ) + (float(config["utility_percentile"]), float(config["margin_percentile"]))
...
def _concentration_audit(trades: pd.DataFrame, arm: str) -> list[dict[str, Any]]:
    ...
    scopes.extend(
        (ticker, trades.loc[trades["ticker"].eq(ticker)])
...
def _run_checkpoint_identity(
    view_manifest: dict[str, Any], *, outer_months: list[str], mode: str
) -> dict[str, Any]:
    return {
        ...
        "master_sha256": MASTER_SHA256,
        "model_spec_sha256": frozen_spec_sha256(),
        "code_hashes": _checkpoint_code_hashes(),
    }
...
def _ensure_run_checkpoint(
    ...
    *,
    ...
    expected = _run_checkpoint_identity(view_manifest, outer_months=outer_months, mode=mode)
    if path.exists():
        stored = json.loads(path.read_text(encoding="utf-8"))
        if stored != expected:
            raise AssertionError(
                "economic run checkpoint identity changed; use a new immutable output target"
            )
        return stored
    unexpected = [item.name for item in output_dir.iterdir() if not item.name.startswith(".")]
    if unexpected:
        raise AssertionError(
            "output target exists without its run checkpoint: " + ",".join(sorted(unexpected))
        )
    _atomic_json(path, expected)
    return expected


def _empty_fold_trades() -> pd.DataFrame:
    return pd.DataFrame(
        columns=[
            *KEY,
            "outer_month",
...
def _fold_checkpoint_identity(
    run_identity: dict[str, Any], *, outer_month: str, ticker: str, arm: str
) -> dict[str, Any]:
    return {
        "schema": FOLD_CHECKPOINT_SCHEMA,
        "experiment": EXPERIMENT,
        ...
        "ticker": ticker,
        "arm": arm,
        "runner_protocol_sha256": run_identity["runner_protocol_sha256"],
        "view_sha256": run_identity["view_sha256"],
        "master_sha256": run_identity["master_sha256"],
        "model_spec_sha256": run_identity["model_spec_sha256"],
        "code_hashes": run_identity["code_hashes"],
    }


def _fold_checkpoint_dir(
    output_dir: Path, *, outer_month: str, ticker: str, arm: str
) -> Path:
    return output_dir / "fold_checkpoints" / outer_month / ticker / arm
...
def _read_fold_checkpoint(
    ...
    try:
        ...
        return None
    stored_identity = {key: manifest.get(key) for key in identity}
    if stored_identity != identity:
        raise AssertionError(
            "fold checkpoint identity changed; use a new immutable output target"
        )
    if manifest.get("status") != "COMPLETE" or manifest.get("atomic_manifest_last") is not True:
        return None
    expected_names = ("selection.csv", "inner_grid.csv", "monthly.csv", "trades.csv")
...
def _compute_fold(
    ...
    *,
    ...
    if (
        ...
        or inner.empty
        or outer.empty
        or not train["month"].max() < inner["month"].min()
        or not inner["month"].max() < outer_month
    ):
    ...
    model = fit_quantile_distribution(
        train,
        features,
    ...
    selected, grid_rows = _select_inner(model.score(inner), grid, inner_months)
    grids = pd.DataFrame(
        [
            {
                "mode": mode,
    ...
    if selected is None:
            ...
            [
                {
                    ...
                    **economic_metrics(outer.iloc[0:0]),
                    "policy_candidates": 0,
            ...
            [
                {
                    **common,
                    "status": "TRADE_OUTER",
                    "passing_grid_count": sum(bool(row["passed"]) for row in grid_rows),
                    **{
                        key: selected[key]
                        for key in (
                            "utility_percentile",
        ...
        trades, candidates = _schedule(model.score(outer), selected)
            ...
            [
                {
                    ...
                    **metric,
                    "policy_candidates": candidates,
                    "scheduler_rejections": candidates - len(trades),
                    "abstention_rate": float(1.0 - candidates / len(outer)),
...
def evaluate(
    ...
    *,
    ...
    run_identity = _ensure_run_checkpoint(
        output_dir, view_manifest, outer_months=outer_months, mode=mode
    )
    fold_frames: list[dict[str, pd.DataFrame]] = []
    reused_folds = 0
    built_folds = 0
    total_folds = len(outer_months) * 3 * 2
    fold_number = 0
    for outer_month in outer_months:
        for ticker in ("SPXW", "QQQ", "SPY"):
            for arm in ("X0", "X1"):
                fold_number += 1
                identity = _fold_checkpoint_identity(
                    run_identity, outer_month=outer_month, ticker=ticker, arm=arm
                )
                directory = _fold_checkpoint_dir(
                    output_dir, outer_month=outer_month, ticker=ticker, arm=arm
                )
                frames = _read_fold_checkpoint(directory, identity)
                if frames is None:
                    frames = _compute_fold(
                        data,
                        view_manifest,
                        outer_month=outer_month,
                        ticker=ticker,
                    ...
                    _write_fold_checkpoint(directory, identity, frames)
                    built_folds += 1
                    checkpoint_status = "built"
                else:
                    reused_folds += 1
                    checkpoint_status = "reused"
                fold_frames.append(frames)
                print(
                    f"[fold {fold_number}/{total_folds}] {outer_month} {ticker} {arm} "
                    f"checkpoint={checkpoint_status}",
                    flush=True,
                )

    selections = pd.concat(
        [frames["selection.csv"] for frames in fold_frames], ignore_index=True
    )
    grids = pd.concat(
        [frames["inner_grid.csv"] for frames in fold_frames], ignore_index=True
    )
    monthly = pd.concat(
        [frames["monthly.csv"] for frames in fold_frames], ignore_index=True
    )
    ...
    nonempty_trades = [frames["trades.csv"] for frames in fold_frames if not frames["trades.csv"].empty]
    trades = pd.concat(nonempty_trades, ignore_index=True) if nonempty_trades else _empty_fold_trades()
        ...
        summaries.append(
            {
                ...
                **pooled,
                ...
                "profit_concentration_top5_trades": top_trades,
                "profit_concentration_top5_days": top_days,
                "all_ticker_month_gates_pass": all_pass,
    ...
    result = {
        "schema": "h_tpovalue1_executable_v1_results",
        "experiment": EXPERIMENT,
        "mode": mode,
        ...
        "verdict": (
            ...
            if mode == "development"
                ...
                "BASE_ECONOMIC_PASS_PENDING_LIVE_PARITY_STRESS"
                if base_economic_pass
                else "FAILED_ECONOMIC"
            )
        ),
        "live_parity": "BLOCKED_IMPLEMENTATION",
        "holdout_2026_opened": False,
        ...
        "checkpoint_contract": {
            ...
            "folds_built_this_run": built_folds,
            "folds_reused_this_run": reused_folds,
        },
    }
    outputs = {
        ...
        "portfolio_summary.csv": summary_frame,
        "concentration_by_ticker.csv": concentration_frame,
        "outer_trades.csv": trades,
    }
...
def main() -> int:
    ...
    data, manifest = load_modeling_data(
        ROOT / args.view, ROOT / args.view_manifest, maximum_date=maximum_date
    )
    if args.mode == "development":
    ...
    result = evaluate(
        data, manifest, outer_months=outer_months, output_dir=output, mode=args.mode
    )

...
if (-not $SkipJepaTrain) {
  Invoke-PythonStep "Train XInput Market JEPA" @(
    ...
    "--device", "cuda"
  ) (Join-Path $ResultsRoot "01_train_xinput_jepa.log")
}

if (-not $SkipAppend) {
  Invoke-PythonStep "Append XInputJEPA features to March cutoff" @(
  ...
  Invoke-PythonStep "Append XInputJEPA features to full backtest data" @(
    (Join-Path $ScriptDir "append_xinput_jepa_features.py"),
...
if (-not $SkipGbtTrain) {
  foreach ($Ticker in $Tickers) {
    ...
    Invoke-PythonStep "Train GBT+XInputJEPA $Ticker" $TrainGbtJepaArgs (Join-Path $ResultsRoot "04_train_gbt_xjepa_$Ticker.log")
    ...
    Invoke-PythonStep "Train XInputJEPA-only GBT $Ticker" $TrainJepaOnlyArgs (Join-Path $ResultsRoot "05_train_gbt_xjepa_only_$Ticker.log")
  }
}
...
if (-not $SkipBacktests) {
  $Runs = @(
    ...
    @("gbt_xjepa", $JepaBacktestDataPath, $GbtJepaModelPath, $GbtJepaNormPath, ""),
    @("xjepa_only", $JepaBacktestDataPath, $GbtJepaOnlyModelPath, $GbtJepaOnlyNormPath, ""),
    @("gbt_xjepa_permuted", $JepaBacktestDataPath, $GbtJepaModelPath, $GbtJepaNormPath, $JepaFeatureNamesPath)
  )
  foreach ($Run in $Runs) {
    $Label = $Run[0]
    $Data = $Run[1]
    ...
    Invoke-PythonStep "Backtest $Label full" ($ArgsFull + $CommonBacktestArgs) (Join-Path $ResultsRoot "06_backtest_${Label}.log")
    ...
    Invoke-PythonStep "Backtest $Label OOS Apr-May" ($ArgsOos + $CommonBacktestArgs) (Join-Path $ResultsRoot "07_backtest_${Label}_oos.log")
  }
}

if (-not $SkipEvaluate) {
  Invoke-PythonStep "Evaluate XInputJEPA full alpha" @(
    (Join-Path $ScriptDir "evaluate_jepa_alpha.py"),
    "--baseline", (Join-Path $ResultsRoot "baseline_gbt_metrics.json"),
    "--jepa", (Join-Path $ResultsRoot "gbt_xjepa_metrics.json"),
    "--jepa-only", (Join-Path $ResultsRoot "xjepa_only_metrics.json"),
    "--jepa-permuted", (Join-Path $ResultsRoot "gbt_xjepa_permuted_metrics.json"),
  ...
  Invoke-PythonStep "Evaluate XInputJEPA OOS alpha" @(
    (Join-Path $ScriptDir "evaluate_jepa_alpha.py"),
    "--baseline", (Join-Path $ResultsRoot "baseline_gbt_oos_metrics.json"),
    "--jepa", (Join-Path $ResultsRoot "gbt_xjepa_oos_metrics.json"),
    "--jepa-only", (Join-Path $ResultsRoot "xjepa_only_oos_metrics.json"),
    "--jepa-permuted", (Join-Path $ResultsRoot "gbt_xjepa_permuted_oos_metrics.json"),
  ...
  Invoke-PythonStep "Diagnose XInputJEPA feature usage" @(
...
Write-Host "`nXInputJEPA GBT research run complete: $ResultsRoot" -ForegroundColor Green

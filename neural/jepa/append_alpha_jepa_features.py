...
from neural.jepa.model import load_alpha_model
...
def infer_batches(model, contexts: np.ndarray, batch_size: int, device: torch.device):
    z_out, pred_out, prob_out = [], [], []
    model.eval()
    with torch.no_grad():
        for start in range(0, len(contexts), batch_size):
            batch = torch.from_numpy(contexts[start : start + batch_size]).to(device).float()
            z, pred, logits = model(batch)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Append AlphaJEPA v2 live-safe features.")
    ...
    model = load_alpha_model(model_dir, map_location="cpu")
    ...
    for i in range(z_dim):
        feature_data[f"ajepa_z_{i:02d}"] = np.zeros(n, dtype=np.float32)
    scalar_cols = [
    ...
    for _, group in work.groupby(["ticker", "date"], sort=False):
        ...
        if len(contexts) == 0:
            continue
        z, pred, probs = infer_batches(model, contexts, args.batch_size, device)
        ...
        for pos in range(len(group)):
            ...
            if not valid[pos]:
                continue
            feature_data["ajepa_context_valid"][out_i] = 1.0
            for zi in range(z_dim):
                feature_data[f"ajepa_z_{zi:02d}"][out_i] = z_by_pos[pos, zi]
            if pos >= 1 and valid[pos - 1]:
                feature_data["ajepa_latent_velocity"][out_i] = float(np.linalg.norm(z_by_pos[pos] - z_by_pos[pos - 1]))
            if pos >= 2 and valid[pos - 1] and valid[pos - 2]:
                v1 = z_by_pos[pos] - z_by_pos[pos - 1]
                v0 = z_by_pos[pos - 1] - z_by_pos[pos - 2]
                feature_data["ajepa_latent_accel"][out_i] = float(np.linalg.norm(v1 - v0))

            short_idx = [h_to_idx.get(h, -1) for h in (1, 3, 6)]
            long_idx = [h_to_idx.get(h, -1) for h in (12, 24, 36)]
            feature_data["ajepa_pred_dispersion_short"][out_i] = pairwise_dispersion(pred_by_pos[pos], short_idx)
            feature_data["ajepa_pred_dispersion_long"][out_i] = pairwise_dispersion(pred_by_pos[pos], long_idx)
            for h, col in [
                (6, "ajepa_lagged_pred_30m_err"),
                (12, "ajepa_lagged_pred_60m_err"),
                (36, "ajepa_lagged_pred_180m_err"),
            ]:
                hi = h_to_idx.get(h)
                src = pos - h
            ...
            feature_data["ajepa_prob_short"][out_i] = float(p[0])
            feature_data["ajepa_prob_hold"][out_i] = float(p[1])
            feature_data["ajepa_prob_long"][out_i] = float(p[2])
            feature_data["ajepa_trade_confidence"][out_i] = float(max(p[0], p[2]))
            feature_data["ajepa_direction_score"][out_i] = float(p[2] - p[0])
            feature_data["ajepa_entropy"][out_i] = entropy

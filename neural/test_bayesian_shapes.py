...
Validates that model outputs and loss function have correct shapes with 93 features.
...
def test_feature_columns():
    ...
    # Sanity check for recent expansion (approximate)
    assert INPUT_SIZE >= 80, f"Expected >80 features after expansion, got {INPUT_SIZE}"
    ...
    confluences = [c for c in FEATURE_COLUMNS if c.startswith("confluence_")]
    engineered = [c for c in FEATURE_COLUMNS if any(x in c for x in ["ratio", "change", "momentum", "magnet"]) and not c.startswith("wk_")]

    print(f"  Weekly features: {len(wk_greeks)}")
    print(f"  Divergence features: {len(divergence)}")
    print(f"  Confluence features: {len(confluences)}")
    print(f"  Engineered features: {len(engineered)}")
    
    assert len(wk_greeks) >= 10, "Expected at least 10 weekly features"
    assert len(divergence) >= 4, "Expected at least 4 divergence features"
    assert len(confluences) >= 5, "Expected at least 5 confluence features"
    assert "net_delta" in FEATURE_COLUMNS, "Missing net_delta"
    assert "dist_to_max_dgex" in FEATURE_COLUMNS, "Missing dist_to_max_dgex"
    assert "net_vega" in FEATURE_COLUMNS, "Missing net_vega"
    assert "net_vomma" in FEATURE_COLUMNS, "Missing net_vomma"
    assert "vega_gamma_ratio" in FEATURE_COLUMNS, "Missing vega_gamma_ratio"
    assert "confluence_ib_high_max_gamma" in FEATURE_COLUMNS, "Missing confluence feature"
    print("  PASSED ✓")

...
- Diagnóstico ya implementado/predeclarado, aún no ejecutado en este checkpoint: `PAIRWISE_INNER_GRID_FAILURE_DECOMPOSITION_V1`. Reentrena exactamente los heads V1 pero no calcula scores/labels outer; persiste `12.474` filas (`99×2×63`) y passes separados de PF/WR/trades/PnL/hold. Runner `run_pairwise_inner_grid_diagnostic_v1.ps1`; tests `3 passed`.
- Corrección crítica posterior: el auditor reveló `hold_pass=0/12.474` porque V1 restaba el minuto de entrada a `opt_exit_minutes`, que ya es duración elapsed. Dataset/builder/scheduler confirman 30–180m y cero labels válidas <30m. Al recalcular las otras cuatro gates aparecen 16 configs válidas antes de hold (15 C0 SPY, 1 P1 SPY; cuatro folds). La afirmación económica `0/99 por señal` queda invalidada; la ciencia model-level no cambia. V1r1 está predeclarado en `PAIRWISE_V1R1_HOLD_DURATION_CORRECTION.md`, output nuevo, aún sin ejecutar en este checkpoint.
...
- Resultado physics skip: 55/99 BA wins, mediana `+0,0039`, `p=0,3199`; Spearman positivo 61/99 pero 2024 delta negativo. QQQ mejora model-level (BA mediana `0,522`, delta `+0,0385`), SPXW/SPY no. Outer physics: QQQ 38 trades/WR `34,21%`/PF `0,4086`/`-8,6909R`; SPY 19/`36,84%`/`0,8101`/`-1,2436R`. Ninguna celda pasa. `REJECTED`; no probar más sub-bloques/features sobre este OOS.
- Conclusión actual honesta: no existe candidato nuevo live-safe. JEPA productivo permanece intacto, pero sus métricas históricas siguen contaminadas por selección 2026 y no son reemplazadas por estos resultados. Para evitar sobreajuste, el siguiente avance válido requiere datos realmente nuevos/holdout posterior o una fuente de alpha causal nueva predeclarada; no más arquitecturas, pesos, thresholds o feature mining en 2022–2025.
- La fuente nueva wall/IB se probó explícitamente porque estaba ausente del physics skip. Join exacto: 86.729 eventos ask→bid 202208–202512, hashes `d3c37b5...a408` y `5f908e1...13b5`, cero 2026. E1 magnet/rejection/acceptance: PF SPXW/QQQ/SPY `0,865/0,832/0,899`. V1r1 corrigió un bug semántico real (rejection sin pierce; solo 29–37% cruzaban) y mejoró a `0,910/0,845/1,008`, todavía rechazado. Artefactos en `.../_diagnostics/wall_interaction_execquote_v1*`; tests `5 passed`. No retunar estados/horas por ticker. La causa restante es informacional y de contrato: se almacenan distancias, pero no fuerza/concentración/persistencia/migración del wall ni max/min delta exposure por strike. El siguiente dataset debe construir ese wall state desde ThetaData, reproducirlo live y validar primero labels de magnet/reject/accept del subyacente a 30–180m; solo si existe separabilidad se vuelve a side/payoff de opciones. Producción y junio siguen intactos.

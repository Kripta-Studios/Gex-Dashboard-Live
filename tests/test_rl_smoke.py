...
# 1. Config imports
print("\n[1/4] Testing imports...")
from rl.config import RL_CONFIG, STRIKE_BUCKETS, HARD_EXITS, TOTAL_STATE_DIM
from rl.rewards import compute_step_reward, compute_terminal_reward
from rl.utils import RunningMeanStd, augment_state, CurriculumScheduler, RolloutBuffer
...
r_win = compute_terminal_reward(0.40, 0.3, 0.30, 0.80, "agent_exit")
r_loss = compute_terminal_reward(-0.30, 0.5, 0.50, 0.70, "hard_stop_loss")
...
assert diff[:162].sum() > 0, "Market features should change"
assert diff[162:].sum() == 0, "Position+MLP should NOT change"
print("  ✓ Augment: market noised, position untouched")

curriculum = CurriculumScheduler(total_updates=100)
c1 = curriculum.get_min_confidence(10)
c2 = curriculum.get_min_confidence(50)
c3 = curriculum.get_min_confidence(90)

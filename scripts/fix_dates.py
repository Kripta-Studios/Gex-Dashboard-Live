...
def run_concurrently(folder_name, file_pattern, worker_func, workers=MAX_WORKERS):
    ...
    print(f"\n" + "="*60)
    print(f"🚀 PROCESANDO {len(files)} ARCHIVOS EN '{folder_name}' CON {workers} WORKERS")
    print("="*60)
...
if __name__ == "__main__":
    ...
    run_concurrently("json_data", "*.json", process_greek_file, workers=20)
    run_concurrently("ib_charts", "ib_data_*.json", process_ib_file, workers=1)
    run_concurrently("ib_backtest", "ib_data_*.json", process_ib_file, workers=1)
    run_concurrently("fourier", "fourier_data_*.json", process_fourier_file, workers=1)

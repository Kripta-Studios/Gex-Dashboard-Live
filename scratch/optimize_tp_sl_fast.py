...
def main():
    ...
    # Take a sample of 80 days of QQQ and 80 days of SPX
    qqq_files = [f for f in files if "QQQ" in f][:80]
    spx_files = [f for f in files if "SPXW" in f][:80]
    
    files_to_process = qqq_files + spx_files
    print(f"Processing {len(files_to_process)} days of 1-minute OHLC data...")
    
    all_res = []
    with ProcessPoolExecutor() as executor:
        for res in executor.map(process_file, files_to_process):
            all_res.extend(res)

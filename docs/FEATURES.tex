...
The PPO agent observes an extended 175-dimensional state vector $\bs_t$ that appends 12 position-management features to the base 163-dimensional market feature vector:
\begin{equation}
    \bs_t = \left[\bx_t \;\Big|\; P_L,\; M_{\text{cap}},\; D_{\text{open}},\; \Delta_{\text{strike}},\; \Gamma_{\text{pos}},\; P_{\text{bid}},\; P_{\text{ask}},\; P_{\text{entry}},\; r_{\text{pos}},\; t_{\text{hold}},\; c_{\text{GBT}},\; d_{\text{GBT}} \right] \in \R^{175}
    \label{eq:rl_state}
\end{equation}
where:
\begin{itemize}
    \item $P_L \in \R$ — unrealized P\&L as a fraction of premium paid.
    \item $M_{\text{cap}} \in [0, 1]$ — fraction of available capital remaining.
    \item $D_{\text{open}} \in [0, 1]$ — normalized hold duration.
    \item $\Delta_{\text{strike}}$ — delta of the current position's strike.
    \item $\Gamma_{\text{pos}}$ — gamma of the current position.
    \item $P_{\text{bid}}, P_{\text{ask}}, P_{\text{entry}}$ — current bid, ask, and entry premium.
    \item $r_{\text{pos}} = P_{\text{mid}} / P_{\text{entry}} - 1$ — current mark-to-market return.
    \item $t_{\text{hold}}$ — minutes since entry.
    \item $c_{\text{GBT}}$ — GBT confidence score at signal time.
    \item $d_{\text{GBT}} \in \{-1, +1\}$ — GBT direction (SHORT/LONG).
\end{itemize}

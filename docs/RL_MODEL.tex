...
Each episode begins when the GBT emits a directional signal with $c_t \geq c_{\min} = 0.55$. The RL agent's state space $\mathcal{S}$ is the 175-dimensional vector defined in Section~6 of the Features document:
\begin{equation}
    \bs_t = \left[\bx_t^\top \;\Big|\; P_L,\; M_{\text{cap}},\; D_{\text{open}},\; \Delta_{\text{strike}},\; \Gamma_{\text{pos}},\; P_{\text{bid}},\; P_{\text{ask}},\; P_{\text{entry}},\; r_{\text{pos}},\; t_{\text{hold}},\; c_{\text{GBT}},\; d_{\text{GBT}}\right]^\top \in \R^{175}
    \label{eq:state}
\end{equation}
where $\bx_t \in \R^{163}$ is the shared market feature vector from the GBT, and the remaining 12 components encode the position's internal state (see Features document for definitions). Prior to any position entry, position-specific components are set to zero.

\begin{definition}[Episode Phases]
Each episode progresses through three phases:
\begin{equation}
    \underbrace{\texttt{PRE\_ENTRY}}_{\substack{\text{up to 15 min} \\ \text{``sniper'' window}}} \;\longrightarrow\; \underbrace{\texttt{IN\_POSITION}}_{\substack{\text{hold/exit decisions} \\ \text{every 5 min}}} \;\longrightarrow\; \underbrace{\texttt{CLOSED}}_{\text{terminal}}
...
\textbf{PRE\_ENTRY (sniper mode):}
\begin{equation}
    \mathcal{A}_{\text{entry}} = \{\textsc{wait},\; \textsc{enter\_delta\_low},\; \textsc{enter\_delta\_mid},\; \textsc{enter\_delta\_high}\}
    \label{eq:action_entry}
\end{equation}
The three entry actions correspond to three delta buckets of the real options chain:
\begin{align}
    \textsc{enter\_delta\_low}  &: |\Delta| \in [0.20, 0.35) \quad \text{(OTM)} \\
    \textsc{enter\_delta\_mid}  &: |\Delta| \in [0.35, 0.55) \quad \text{(near-ATM)} \\
    \textsc{enter\_delta\_high} &: |\Delta| \in [0.55, 0.75] \quad \text{(ITM)}
\end{align}
For each delta bucket, the system selects the actual strike from the real options chain as the one closest to the bucket's midpoint delta, given the GBT's directional signal (call for LONG, put for SHORT).
...
The \texttt{PPOAgent} is implemented as an actor-critic network with a shared feature extractor. The architecture contains approximately 187,348 trainable parameters:
...
The shared trunk processes the 175-dimensional state vector, producing a latent representation that is subsequently branched into the phase-conditioned actor heads and the value function critic. The actor selects from $\mathcal{A}_{\text{entry}}$ during PRE\_ENTRY and from $\mathcal{A}_{\text{pos}}$ during IN\_POSITION; the critic outputs a scalar value estimate $V_{\bph}(\bs_t)$ used in the GAE advantage computation.
...
\subsection{Entry Timing Bonus}

The 15-minute sniper window allows the agent to improve entry price relative to the GBT signal timestamp. Favorable price improvement during PRE\_ENTRY generates a small bonus:
\begin{equation}
    R^{\text{entry}} = \min\!\left(5 \cdot \Delta_{\text{price}},\; 0.3\right) \cdot \mathbf{1}[\Delta_{\text{price}} > 0]
    \label{eq:r_entry}
\end{equation}
where $\Delta_{\text{price}} = (P_{\text{entry\_actual}} - P_{\text{signal\_time}}) / P_{\text{signal\_time}}$ measures the improvement achieved by waiting for a more favorable entry price. The capping at $0.3$ prevents the agent from exclusively optimizing entry timing at the cost of missing the trade entirely.
...
\begin{equation}
    R_t =
    \begin{cases}
        R^{\text{PnL}} + R^{\text{capture}} + R^{\text{stop}} + R^{\text{entry}} & \text{if } t = t_{\text{exit}} \text{ (terminal)} \\
...
Maximum profit & $P_L \geq +3.00$ & Capture extraordinary gains (300\% of premium) \\
...
\begin{tabular}{llcc}
\toprule
\textbf{Phase} & \textbf{Training Steps} & \textbf{Min Confidence $c_{\min}$} & \textbf{Rationale} \\
\midrule
Phase 1: High confidence & Steps 1--50 & 0.70 & Learn on cleanest signals first \\
Phase 2: Medium confidence & Steps 51--120 & 0.60 & Expand to moderate signals \\
Phase 3: Full distribution & Steps 121+ & 0.55 & Learn full production range \\
\bottomrule
\end{tabular}
...
\begin{enumerate}
    ...
    \item The RL agent steps through PRE\_ENTRY and IN\_POSITION phases using live options quotes from the real-time feed.

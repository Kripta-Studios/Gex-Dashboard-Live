...
The unified PowerShell orchestrator executes Stages~1--5 automatically:
\begin{lstlisting}[language=bash]
.\neural\run_pipeline.ps1 -gbt
\end{lstlisting}
This script orchestrates data collection, walk-forward training, episode generation, RL preprocessing, and PPO training. The script accepts granular flags (e.g., \texttt{-rl}, \texttt{-bt}, \texttt{-v}) to skip to specific pipeline stages.
...
\begin{lstlisting}[language=bash]
python neural/collect_training_data_spx_qqq.py \
  --start 20220801 \
  --end 20260330 \
  --workers 20 \
  --tickers SPX QQQ \
...
python neural/train_walkforward.py \
  --data training_data/training_data_spx_qqq.parquet \
  --model-size small \
  --train-months 9 \
  --test-months 1 \
  --ensemble 5 \
...
df = pd.read_parquet('training_data/training_data_spx_qqq.parquet')
...
python run_preprocess.py \
  --training-data ../training_data/training_data_spx_qqq.parquet \
...
Train the RL agent (181,010-parameter PPO network):
\begin{lstlisting}[language=bash]
cd neural

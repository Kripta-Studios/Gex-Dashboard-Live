...
def apply_custom_style(ax, fig, title, xlabel, ylabel):
    ...
    # Títulos y Etiquetas
    ax.set_title(title.replace("<br>", " "), color=c["color_text"], fontsize=c["title_size"], pad=25)
    ...
    # Ejes (Ticks)
    ax.tick_params(axis='x', colors=c["color_text"], labelsize=c["tick_size"], width=2, length=8)
    ax.tick_params(axis='y', colors=c["color_text"], labelsize=c["tick_size"], width=2, length=8)
    ...
    # Grid (Cuadrícula)
    ax.grid(True, color=c["color_grid"], linestyle='-', linewidth=c["grid_width"], alpha=c["grid_alpha"])

async def plot_greeks_table(df, today_ddt, today_ddt_string, monthly_options_dates, spot_price, from_strike, to_strike, levels, totaldelta, totalgamma, totalvanna, totalcharm, totaldgex, totalzomma, zerodelta, zerogamma, call_ivs, put_ivs, exp, ticker, lower_bound, upper_bound, prev_close_price, greek_filter=None):
    if not isinstance(df, pd.DataFrame) or df.empty:
        return [], []
    
    filenames = []
    alerts = []
    GREEKS = [greek_filter] if greek_filter else ["delta", "gamma", "vanna", "charm", "dgex", "zomma"]
    ...
    for greek in GREEKS:
        for value in VISUALIZATIONS[greek]:
            if "Absolute" not in value: continue
            try:
                ...
                pivot_table = df_filtered.pivot_table(index='strike_price', columns='expiration_date', values=metric, aggfunc='sum')
                pivot_table = pivot_table.fillna(0)
                if pivot_table.empty: continue
                ...
                if len(signs) > 0:
                    runs = []
                    current_sign = signs[0]; start = 0
                    for j in range(1, len(signs)):
                        if signs[j] != current_sign:
                            runs.append((current_sign, start, j-1))
                            current_sign = signs[j]; start = j
                    runs.append((current_sign, start, len(signs)-1))
                    for r in range(1, len(runs)):
                        prev_run = runs[r-1]; curr_run = runs[r]
                        if (prev_run[2]-prev_run[1]+1) >= 2 and (curr_run[2]-curr_run[1]+1) >= 2:
                            idx1 = prev_run[2]; idx2 = curr_run[1]
                ...
                table_pickle_path = Path(f"pickles/{ticker}/{exp}/{greek}/table_history.pkl")
                ...
                ax.set_title(f"{ticker} {value} Heatmap, {today_ddt_string}", color='white', fontsize=STYLE_CONFIG["title_size"], pad=30)
                ax.set_xlabel("Expiration Date", color='white', fontsize=STYLE_CONFIG["label_size"], labelpad=20)
                ax.set_ylabel("Strike Price", color='white', fontsize=STYLE_CONFIG["label_size"], labelpad=20)
                ...
                for i in range(len(strikes)):
                    for j in range(len(expirations)):
                        ...
                        if not np.isnan(value2text):
                            ...
                            text_str = f"$ {value2text*100000:,.2f}k" if value2text % 1 != 0 else f"$ {int(value2text)*100000:,d}k"
                            ax.text(j, i, text_str, ha='center', va='center', color=text_color, fontsize=11)
                ...
                # Helper índice Y
                def get_y(val): return np.abs(np.array(strikes) - val).argmin()

                # 1. Spot
                spot_idx = get_y(spot_price)
                ax.axhline(y=spot_idx, color='white', linestyle='--', linewidth=2, label=f'Spot: {spot_price:.2f}')
                ...
                if not pd.isna(max_pos):
                    ...
                    ax.axhline(y=max_pos_idx, color="lime", linestyle="--", linewidth=2.5, label=f"Max Pos: {max_pos:.0f}")
                ...
                if not pd.isna(max_neg):
                    ...
                    ax.axhline(y=max_neg_idx, color="red", linestyle="--", linewidth=2.5, label=f"Max Neg: {max_neg:.0f}")
                ...
                if rising_strike:
                    ax.axhline(y=get_y(rising_strike), color="cyan", linestyle=":", linewidth=3, label=f"Rising: {rising_strike:.0f}")
                if falling_strike:
                    ax.axhline(y=get_y(falling_strike), color="orange", linestyle=":", linewidth=3, label=f"Falling: {falling_strike:.0f}")

                # 4. Flip
                if flip_val:
                    ax.axhline(y=get_y(flip_val), color="cyan", linestyle="--", linewidth=2, label=f"Flip: {flip_val:.2f}")
                ...
                cbar.set_label(f"{name} Exposure", color='white', fontsize=STYLE_CONFIG["label_size"])
                tick_vals = [V_min * 0.8, 0, V_max * 0.8]
                cbar.set_ticks(tick_vals)
                cbar.ax.set_yticklabels([f"{x:.2f}" if abs(x) < 1 else f"{int(x)}" for x in tick_vals])
                cbar.ax.yaxis.set_tick_params(color='white', labelcolor='white', labelsize=STYLE_CONFIG["tick_size"])
...
async def plot_greeks_histogram(df, today_ddt, today_ddt_string, monthly_options_dates, spot_price, from_strike, to_strike, levels, totaldelta, totalgamma, totalvanna, totalcharm, totaldgex, totalzomma, zerodelta, zerogamma, call_ivs, put_ivs, exp, ticker, lower_bound, upper_bound, prev_close_price, greek_filter=None):
    ...
    PLOT_DIR = "plots"
    timestamp = datetime.datetime.now(ZoneInfo("America/New_York")).strftime("%Y%m%d_%H%M%S")
    colors = {"total": "#89CFF0", "call": "#90EE90", "put": "#FF7F7F", "spot_line": "#C0C0C0"}
    ...
    for greek in GREEKS:
        for value in VISUALIZATIONS[greek]:
            try:
                ...
                # Validaciones
                if date_condition:
                    if not isinstance(from_strike, (int, float)) or not isinstance(to_strike, (int, float)): continue
                    ...
                    step = np.min(strike_steps[strike_steps > 0]) if len(strike_steps) > 0 else 1
                    lower_strike_calc = max(np.floor(lower_bound / step) * step, df_agg.index.min())
                    upper_strike_calc = min(np.ceil(upper_bound / step) * step, df_agg.index.max())
                    strikes = np.arange(lower_strike_calc, upper_strike_calc + step, step)
                ...
                if "Calls/Puts" in value or value == "Implied Volatility Average":
                    ...
                    call_ivs_data = call_ivs.get(key); put_ivs_data = put_ivs.get(key)
                else: call_ivs_data = put_ivs_data = None

                name = value.split()[1] if "Absolute" in value else value.split()[0]
                
                # --- ETIQUETA EJE Y DINÁMICA ---
                ylabel_text = f"{name} Exposure"
                if name == "Gamma": ylabel_text += " (gamma / 1% move)"
                elif name == "Delta": ylabel_text += " (delta / 1% move)"
                elif name == "Vanna": ylabel_text += " (vanna / 1% IV move)"
                elif name == "Charm": ylabel_text += " (charm / day)"
                elif name == "Zomma": ylabel_text += " (zomma / 1% IV move)"
                elif name == "Dgex" : ylabel_text += " (dgex / 1% move)"
                # --- FIGURE PRIVADO ---
                ...
                apply_custom_style(ax, fig, title, "Strike" if date_condition else "Date", ylabel_text)
                ...
                if "Absolute" in value:
                    ...
                    pickle_path = Path(f"pickles/{ticker}/{exp}/{greek}/agg_by_strike.pkl")
                    pickle_path.parent.mkdir(parents=True, exist_ok=True)
                    rising_strike = None; falling_strike = None
                    ...
                    for r in range(1, len(runs)):
                        prev_run = runs[r-1]; curr_run = runs[r]
                        if (prev_run[2]-prev_run[1]+1) >= 2 and (curr_run[2]-curr_run[1]+1) >= 2 and prev_run[0]!=0 and curr_run[0]!=0:
                            idx1 = prev_run[2]; idx2 = curr_run[1]
                            s1, v1 = agg_by_strike.index[idx1], agg_by_strike.iloc[idx1]
                            s2, v2 = agg_by_strike.index[idx2], agg_by_strike.iloc[idx2]
                    ...
                    if pickle_path.exists():
                        try:
                            ...
                            if ticker == "SPX" and exp == "0dte":
                                try:
                                    if greek == "vanna":
                                        prev_min = previous.idxmin(); curr_min = agg_by_strike.idxmin()
                                        if prev_min != curr_min and spot_price*0.98 < float(curr_min) < spot_price*1.02:
                                            msg = f"**SPX 0DTE Vanna Alert**: Max Negative Vanna shifted from {prev_min} to {curr_min}"
                                            if msg not in alerts: alerts.append(msg)

                                    if greek == "gamma":
                                        prev_max = previous.idxmax(); curr_max = agg_by_strike.idxmax()
                                        prev_min = previous.idxmin(); curr_min = agg_by_strike.idxmin()
                                        ...
                                        if prev_min != curr_min:
                                            ...
                                            if msg not in alerts: alerts.append(msg)
                                except Exception as e: print(f"Error checking shifts: {e}")

                            common = agg_by_strike.index.intersection(previous.index)
                            if not common.empty:
                                diff = agg_by_strike.loc[common] - previous.loc[common]
                                if not diff.empty and diff.abs().max() > 0:
                                    rising_strike = diff.idxmax(); falling_strike = diff.idxmin()
                        except: pass
                    ...
                    ax.bar(df_agg.index, agg_by_strike, align='edge', width=step*0.9, label=f"{name} Exposure", alpha=0.9, color=colors["total"])
                    
                    if not pd.isna(max_pos):
                        ax.axvline(x=max_pos, color="lime", linestyle="--", linewidth=2, label=f"Max Pos: {max_pos:.2f}")
                    if not pd.isna(max_neg):
                        ax.axvline(x=max_neg, color="red", linestyle="--", linewidth=2, label=f"Max Neg: {max_neg:.2f}")
                    
                    if rising_strike:
                        ax.axvline(x=rising_strike, dashes=(5,12), color="cyan", linestyle="--", linewidth=3, label=f"Rising: {rising_strike:.0f}")
                    if falling_strike:
                        ax.axvline(x=falling_strike, dashes=(5,12), color="orange", linestyle="--", linewidth=3, label=f"Falling: {falling_strike:.0f}")
                    
                    if zero_strike:
                        ax.axvline(x=zero_strike, color="yellow", linestyle="--", linewidth=2, label=f"Flip: {zero_strike:.2f}")
                ...
                # ==========================================================
                    ...
                    max_pos = agg_by_strike_ref.idxmax(); max_neg = agg_by_strike_ref.idxmin()
                    
                    rising_strike = None; falling_strike = None
                    if pickle_path.exists():
                        try:
                            previous = pd.read_pickle(pickle_path)
                            common = agg_by_strike_ref.index.intersection(previous.index)
                            if not common.empty:
                                diff = agg_by_strike_ref.loc[common] - previous.loc[common]
                                if not diff.empty and diff.abs().max() > 0:
                                    rising_strike = diff.idxmax(); falling_strike = diff.idxmin()
                        except: pass

                    ax.bar(df_agg.index, df_agg[f"call_{name[:1].lower()}ex"] / scale, align='edge', width=step*0.9, label=f"Call {name}", alpha=0.9, color=colors["call"])
                    ax.bar(df_agg.index, df_agg[f"put_{name[:1].lower()}ex"] / scale, align='edge', width=step*0.9, label=f"Put {name}", alpha=0.9, color=colors["put"])
                    
                    if not pd.isna(max_pos): ax.axvline(x=max_pos, color="lime", linestyle="--", linewidth=2, label=f"Max Pos: {max_pos:.2f}")
                    if not pd.isna(max_neg): ax.axvline(x=max_neg, color="red", linestyle="--", linewidth=2, label=f"Max Neg: {max_neg:.2f}")
                    if rising_strike: ax.axvline(x=rising_strike, dashes=(5,12), color="cyan", linestyle="--", linewidth=3, label=f"Rising: {rising_strike:.0f}")
                    if falling_strike: ax.axvline(x=falling_strike, dashes=(5,12), color="orange", linestyle="--", linewidth=3, label=f"Falling: {falling_strike:.0f}")
                ...
                # ==========================================================
                    ...
                    try:
                        ...
                        if min_len > 0:
                            ...
                            y_p = put_ivs_data[:min_len]*100; y_c = call_ivs_data[:min_len]*100
                            ax.plot(x_ax, y_p, label="Put IV", color=colors["put"], linewidth=3)
                            ax.fill_between(x_ax, y_p, alpha=0.3, color=colors["put"])
                            ax.plot(x_ax, y_c, label="Call IV", color=colors["call"], linewidth=3)
                ...
                if date_condition:
                    ...
                    if step < 1: step = 0.5
                    elif 1 <= step < 2: step = 2
                    elif 2 <= step < 2.5: step = 5
                    elif 2.5 <= step < 3: step = 10
                    else: step = (math.floor(step)); step = (10 ** (step-1))*0.4
                    
                    lb_plot = step * np.floor(float(lower_bound) / step)
                    ub_plot = step * np.ceil(float(upper_bound) / step)

                    ax.axvline(x=spot_price, color=colors["spot_line"], linestyle="--", linewidth=1.5, label=f"{ticker} Spot: {spot_price:,.2f}")
                    ax.set_xlim(lb_plot, ub_plot)
                    x_ticks = np.arange(lb_plot, ub_plot + step, step)
                    ax.set_xticks(x_ticks)
                    ax.set_xticklabels([f"{x:.2f}" if step < 1 else f"{int(x)}" for x in x_ticks])
                else:
                ...
                makedirs(path.dirname(filename), exist_ok=True)
                fig.savefig(filename, bbox_inches="tight", facecolor=STYLE_CONFIG["color_bg"], dpi=80)
                plt.close(fig)
                filenames.append(filename)
    ...
    async def _fetch_internal():
        ...
        try:
            ...
            def serialize(obj):
                ...
                if isinstance(obj, (pd.Timestamp, datetime.date, datetime.datetime)): return obj.isoformat()

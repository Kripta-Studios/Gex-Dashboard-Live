...
def load_ib_candles(ticker: str, date_str: str) -> pd.DataFrame:
    """Load ThetaData Parquet candle data for a ticker and date."""
...
def plot_pnl_summary(trades_df: pd.DataFrame, output_path: str = None):
    ...
    for i, ticker in enumerate(tickers):
        ...
        df_ticker['cum_pnl'] = df_ticker['pnl_dollars'].cumsum()
...
def main():
    ...
    for _, row in trades_df.head(25).iterrows():
        pnl_val = row.get('pnl_dollars', 0.0)
        pnl_str = f"+${pnl_val:.0f}" if pnl_val > 0 else f"${pnl_val:.0f}"
        win_str = "✓" if pnl_val > 0 else "✗" if pnl_val < 0 else "-"
        # Handle both old and new column names

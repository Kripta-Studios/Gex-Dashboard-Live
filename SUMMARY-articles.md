...
Actualización 2026-07-11 — Preflight final de PAIRWISE_OPPORTUNITY_AND_SIDE_SELECTION_V1 corregido antes de entrenar. La vista sellada mantiene 97.625 filas `20220103..20251231`, sin 2026, SHA-256 `d3c37b5...a408`. C0/P1 conservan matriz/hash común `fa205765...d50e`. La métrica científica ya compara `p_call_win-p_put_win` frente a `2*p_call-1` en una única máscara outer finita, independientemente de que la policy opere; incluye accuracy, balanced accuracy, ROC-AUC y Spearman, conserva las 99 celdas y marca degeneradas como no favorables. El runner exige modo explícito y separa preflight/real; `-PreflightOnly` tiene un test que impide cualquier llamada a training. Suite previa al commit `27 passed`, compile/parser/diff PASS. El manifiesto final todavía no se ha generado: debe hacerse desde commit limpio y sincronizado, y solo después se ejecutará el experimento congelado con la nueva autorización del usuario. No se abre 2026 ni se toca producción.

Resultado final Pairwise V1: `REJECTED`. La corrida completó 99/99 celdas en 346,5 s, sin 2026. P1 mejora balanced accuracy solo en 57/99 (mediana `+0,004`, `p=0,117`) y Spearman es positivo en 58/99 (mediana `0,0303`); falla los dos umbrales de 60% y empeora QQQ. Tanto C0 como P1 encuentran `0/99` configs inner válidas y abstienen completamente. El oracle-side no causal sí obtiene PF `4,55..11,73`, WR `73,5..86,7%` y todos los meses positivos, mientras reglas CALL/PUT/momentum simples pierden: el cuello sigue siendo extracción causal del lado. No retunar V1 ni volver a otra arquitectura; primero auditar el grid inner completo y después aislar si ponderar/regresar `side_advantage` por magnitud corrige el desalineamiento entre clasificación binaria y payoff.

Actualización: el auditor inner-grid está predeclarado y listo, todavía sin ejecutar. No cambia arquitectura/labels/features/grid y no puntúa outer; materializará `99×2×63=12.474` filas para atribuir la inanición a PF, WR, frecuencia, PnL o hold antes de autorizar el cambio aislado de objetivo.

Corrección posterior: el auditor encontró que V1 interpretó `opt_exit_minutes` (duración elapsed) como minuto absoluto y calculó `duration-entry`, haciendo fallar artificialmente los 12.474 holds. Las labels válidas tienen 30–180m. Sin ese bug hay 16 configuraciones que pasan las otras cuatro gates, todas SPY. V1r1 queda predeclarado como corrección de una sola expresión, sin cambiar arquitectura/modelos/grid; la conclusión científica débil de P1 sigue vigente, pero la conclusión económica de V1 debe sustituirse por V1r1.

Resultado V1r1: corrección reproducida, pero modelo rechazado. Solo SPY selecciona 3 folds C0 y 1 P1. C0 outer queda PF `0,9978`/WR `45,9%`/`-0,04R`; P1 PF `0,4265`/WR `34,8%`/`-4,93R`. SPXW/QQQ siguen sin policy. El grid corregido demuestra que frecuencia no es el límite (P1 pasa volumen en 85–97% de filas), mientras PF/WR casi nunca pasan. La siguiente hipótesis debe alinear el objetivo de side con magnitud de payoff, no cambiar JEPA/arquitectura/cadencia.

Queda predeclarada la prueba exacta de esa hipótesis: mismo P1, ponderando únicamente la loss binaria de side por la magnitud absoluta de ventaja, capped q95 train-only. No añade arquitectura, features ni sweep. Es mecanismo discovery adaptativo; aunque pasara necesitaría un holdout nuevo antes de cualquier live.

Resultado magnitude-weighted: rechazado (`48/99` BA wins, mediana `-0,0001`, `p=0,664`; SPY-only outer PF `0,430`, WR `27,3%`). No probar otros caps. La próxima pregunta aislada es informacional: si las 30 features pairwise omiten superficie física/cross-asset current-time que el dataset y live contract ya contienen; se añadirá solo al side head, sin cambiar arquitectura.

El side physics skip queda predeclarado/listo: 154 features current-time solo en side frente a 30 control; opportunity y todo el protocolo permanecen idénticos. Smoke real y tests pasan; todavía no hay resultado full en este checkpoint.

Resultado physics skip: rechazado. QQQ mejora BA model-level, pero outer PF cae a `0,409`; SPY PF `0,810`; SPXW abstiene. Globalmente 55/99 BA wins, p=`0,320`, año 2024 negativo. No hacer feature mining adicional sobre 2022–2025. La evidencia conjunta descarta arquitectura, ponderación y bloque físico como soluciones suficientes; hace falta holdout/datos nuevos o una fuente causal nueva, no otra iteración JEPA sobre el mismo OOS.

Actualización wall-interaction 2026-07-11: la nueva fuente causal señalada por el usuario sí estaba ausente del physics skip: este no contenía distancias a max/min gamma/DGEX, zero-gamma, IB previos ni estados de toque/rechazo/aceptación. Se unieron 86.729 eventos `executable_quote` 202208–202512 con paridad uno-a-uno y spot idéntico. La regla fija E1 falló (PF SPXW/QQQ/SPY `0,865/0,832/0,899`). La atribución reveló un defecto semántico: solo 29–37% de los rechazos habían cruzado el nivel. V1r1 exigió pierce real y mejoró a `0,910/0,845/1,008`, pero sigue rechazado. Existe señal parcial: acceptance CALL acierta dirección spot ~58–63% a 30m en varias celdas y magnet CALL/rejection PUT son rentables en muestras pequeñas, insuficientes para 18/mes. No retunar. El gap real es que el contrato solo guarda localización: faltan magnitud/concentración/persistencia/migración del wall y max/min delta exposure por strike. El siguiente build debe modelar estado de wall y validar primero la física del subyacente, no otra arquitectura.
...
Actualización 2026-07-11 15:45 CEST — Auditoría de reproducibilidad finalizada con éxito (test PASS). Se demostró equivalencia interna absoluta para C0 (trades hash `9a59c1f7...7ab` en control y replay exacto) y coincidencia matemática completa en reconstrucción de thresholds (`THRESHOLD_MATCH` clasificado para todos los folds audited de R1-R4, con umbral de QQQ 202602 R1 verificado en exactamente `0.012700021`). Se expandió `freeze_fold_policy_artifact` para persistir los metadatos del gate, commit SHA de Git, y hash del dataset en `fold_policy.json`, añadiéndose unit tests que prueban esta serialización, causal checks de minutos y filtrado del scheduler contrafactual. Resultados guardados en `research_papers/JEPA/results/_diagnostics/regime_gate_ablation_v1_reproducibility_audit/` con manifiesto e informes.

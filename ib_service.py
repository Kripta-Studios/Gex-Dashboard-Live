...
def generate_ib_chart(ticker, df, levels, date_str):
    ...
    if current_price > ib_high:
        ...
        for i, ext in enumerate(exts):
            ...
            ax.axhline(
                fib_level, color=colors_fib[i], linestyle=":", linewidth=1, alpha=0.8
            )
            ax.text(
                0.01,
                fib_level,
                f"FIB {ext} ({fib_level:.2f})",
                color=colors_fib[i],
                fontsize=8,
                transform=trans,
                va="bottom",
    ...
    sorted_greeks = sorted(
        [(k, v) for k, v in levels.items() if not pd.isna(v)], key=lambda x: x[1]
    )
...
class IBBot(discord.Client):
    ...
    def process_tickers(self):
        ...
        print(
            f"--- Iniciando ciclo de procesamiento IB {datetime.now().strftime('%H:%M:%S')} ---",
            flush=True,
        )
        if SERVER_TZ:
        ...
        for ticker in TICKERS_TO_TRACK:
            try:
                ...
                if df_spot.empty or not latest_file:
                    continue

...
async function init() {
  ...
  updateMarketSpots();
  updateNYTime();
  startAutoRefresh(); // Start auto-refresh in LIVE mode
}

// --- AUTO-REFRESH CONTROL ---
// Controls automatic data refresh - only runs when in LIVE mode (currentHistoryTimeEST === null)
function startAutoRefresh() {
  stopAutoRefresh(); // Clear any existing interval first

  // Only start auto-refresh if we're in LIVE mode
  if (currentHistoryTimeEST === null) {
    refreshIntervalId = setInterval(() => {
      // Double-check we're still in LIVE mode before refreshing
      if (currentHistoryTimeEST === null) {
        refreshDashboard();
      }
...
function loadHistoryNow() {
  ...
  refreshDashboard()
    .then(() => {
      ...
      btn.disabled = false;
      // Update data time display immediately
      updateNYTime();
    })
    .catch((error) => {
...
function goLive() {
  ...
  // 4. RESTART auto-refresh since we're back in LIVE mode
  startAutoRefresh();
}
...
function createChartPanel(chartObj, index) {
  ...
  rows.forEach((row) => {
    ...
    div.innerHTML = `
                <div class="y-axis-label" style="${labelStyle}">${row.strike
      }</div>
                <div class="bar-area">
...
function updateNYTime() {
  ...
  currentTab.charts.forEach(chart => {
    ...
    // Prioridad 1: Campo today_ddt_string (Tablas Heatmap - formato: "2026 Jan 23, 03:30 PM EST")
    if (chart.data && chart.data.today_ddt_string) {
      // Extraer la hora de "2026 Jan 23, 03:30 PM EST"
    ...
    // Prioridad 2: Fourier - data es un array directamente (formato datetime: "YYYY-MM-DD HH:MM:SS")
    else if (chart.data && Array.isArray(chart.data) && chart.data.length > 0) {
      const last = chart.data[chart.data.length - 1];
      if (last.datetime) {
        // Extraer HH:MM de "2026-01-23 15:30:00"
        const timePart = last.datetime.split(' ')[1];
        if (timePart) {
          timeStr = timePart.substring(0, 5); // "15:30"
        }
      }
    }
    // Prioridad 3: IB Charts - data.series es un array (formato time: "HH:MM")
  ...
  // Update data time display with HISTORY indicator if not in LIVE mode
  if (dataEl) {
    if (currentHistoryTimeEST !== null) {
      // HISTORY MODE: Show the time from the data with a visual indicator
      dataEl.innerText = latestStr + " (H)";
      dataEl.style.color = "var(--accent-red)";
    } else {
      // LIVE MODE: Show the time normally
      dataEl.innerText = latestStr;
      dataEl.style.color = ""; // Reset to default color
    }
  }
}
...
// --- BASE refreshDashboard FUNCTION ---
// This is the core function that refreshes all charts in the current tab
async function refreshDashboard() {
  const currentTab = tabs.find(t => t.id === currentTabId);
  if (!currentTab || !currentTab.charts || currentTab.charts.length === 0) return;

  // Filter heatmap charts only (base function handles heatmaps)
  const heatmaps = currentTab.charts.filter(c => !c.type || c.type === 'heatmap');

  // Refresh each heatmap chart with the appropriate data
  for (let chart of heatmaps) {
    const newData = await fetchChartData(chart.inputTicker, chart.inputExp);
    if (newData) {
      chart.data = newData;
    }
  }
...
refreshDashboard = async function () {
  ...
  if (heatmaps.length > 0) {
    ...
    try {
      ...
      // 4. Re-render charts with the new batch data
      renderAllCharts();
      updateNYTime();
    } catch (e) {
      console.error("Batch update failed", e);

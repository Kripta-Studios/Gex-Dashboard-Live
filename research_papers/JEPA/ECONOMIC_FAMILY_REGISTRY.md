...
| EVENT_OPTION_EXECQUOTE_FULL_NESTED_MONTHLY_OVERLAY_V1 | LightGBM CALL/PUT y búsqueda exhaustiva mensual de 1.128.960 overlays sobre ask→bid | `ACTIVE` | predeclarada development-only; seis folds train→select6m→test1m, freeze mensual y ledger solo test |
...
2026. La única familia activa es el selector exhaustivo nested development-only
sobre el parquet ask-to-bid existente; un PASS no convierte 2026 en outer.
GroupDRO quedó cerrado sin ejecución. Producción/live/systemd permanecen
cerrados. Autoridades

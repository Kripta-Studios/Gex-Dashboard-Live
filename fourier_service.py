...
TICKERS_TO_TRACK = [
    "SPX", "SPY", "QQQ", "IWM", "VIX", 
    "AAPL", "MSFT", "AMZN", "META", "MSTR", 
    "TSLA", "SLV", "GLD", "GOOGL", "NVDA", 
    "AMD", "NFLX", "PLTR", "HOOD", "HIMS", "UNH"
]
...
def get_data_for_ticker(ticker, expiry, target_date):
    pattern = os.path.join(DATA_FOLDER_PATH, f"*{ticker}*{expiry}*ExposureData*{target_date}*.json")
...
def detect_turns(y_data):
    ...
    for i in turns_indices:
        if i + 1 >= len(y_data): continue
        current_sign = np.sign(dy[i+1])
        prev_sign = np.sign(dy[i])
        if prev_sign > 0 and current_sign < 0: events.append((i, 'PEAK'))
        elif prev_sign < 0 and current_sign > 0: events.append((i, 'VALLEY'))
    return events
...
def analyze_save_and_plot(ticker, df, date_str):
    ...
    # 1. JSON
    json_df = df.copy()
    json_df['datetime'] = json_df['datetime'].apply(lambda x: x.strftime('%Y-%m-%d %H:%M:%S'))
    ...
    plt.title(f'{ticker} | FOURIER PRICE vs IV | {formatted_date} (NY Time)', 
              color='#e0e0e0', fontsize=14, fontweight='bold', pad=20, loc='left')

    # --- EJE PRECIO (NEON CYAN) ---
    ax1.plot(df['datetime'], df['spot_fft'], color='#00F0FF', linewidth=2, label='Spot Price')
    ...
    # --- EJE IV (NEON MAGENTA) ---
    ax2 = ax1.twinx()
    ax2.plot(df['datetime'], df['iv_fft'], color='#FF00FF', linewidth=2, alpha=0.9, label='Put IV')
    ...
    for idx, tipo in iv_turns:
        ...
        c = '#FF0000' if tipo == 'PEAK' else '#00FF00'
        ax2.scatter(x_val, y_val, color=c, s=40, zorder=10, marker='D', edgecolors='white', linewidth=1)
        ...
        for ticker in TICKERS_TO_TRACK:
            try:
                ...
                if df.empty:
                    ...
                    if ticker == "SPX": 
                        print(f"[DEBUG SPX] No se encontraron registros válidos para hoy ({today_str}).")
                    else:

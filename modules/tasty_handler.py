...
def get_future_ticker(
    symbol: str, current_date: datetime.datetime = None, monthly: bool = None
) -> str:
    """
...
async def get_market_data_async(session, equities=None, options=None):
    return await asyncio.to_thread(
        get_market_data_by_type, session, equities=equities, options=options
    )
...
async def tasty_expirations_strikes(session, options_ticker: list[str]):
    ...
    chains_list = await asyncio.gather(
        *[get_chain_async(session, t) for t in options_ticker]
    )
        ...
        if "/" in i:
            for chain in chains_list:
                for subchain in chain.option_chains:
                    for (
                        expiration
                    ) in subchain.expirations:  # <- NestedFutureOptionChainExpiration
...
async def collect_events(
    streamer, event_type, symbols, greeks_list, symbol_pairs, timeout=2
):
                    ...
                    for idx, d in enumerate(greeks_list):
                        if d.get("symbol") == tasty_symbol:
                            if event_type == Greeks:
                                ...
                                d.update({"open_interest": str(event.open_interest)})
    ...
    if missing:
        print(
            f"Timeout loading {event_type.__name__} events for symbols (not received)"
        )
...
async def main_downloader(
    session, options_requested: OptionsRequest = None, equities_ticker: List[str] = []
) -> Tuple[List, List]:

    if (not isinstance(options_requested, dict)) and options_requested != None:
        raise TypeError(
            """Parameter 'options_requested' must be a dict (TypedDict) with the following string keys : value 
                        tickers: List[str],
                        start_date: datetime.date,
                        end_date: datetime.date,
                        lower_strike: str,
        ...
        equities_spot.append(
            {
                ...
                "time": i.updated_at.astimezone(ZoneInfo("America/New_York")).strftime(
                    "%Y-%m-%d %H:%M:%S"
                ),
            }
        )
                            ...
                            if (
                                ...
                                if strike.call:
                                    ...
                                    if call_entry not in greeks_list:
                                        symbol_pairs.append(
                                            (strike.call, strike.call_streamer_symbol)
                                        )
                                ...
                                if strike.put:
                                    ...
                                    if put_entry not in greeks_list:
                                        symbol_pairs.append(
                                            (strike.put, strike.put_streamer_symbol)
                                        )
    ...
    async with DXLinkStreamer(session) as streamer:
        ...
        await asyncio.gather(
            collect_events(
                streamer, Greeks, tasty_symbols, greeks_list, symbol_pairs, timeout=2
            ),
            collect_events(
                streamer, Summary, tasty_symbols, greeks_list, symbol_pairs, timeout=2
            ),
        )
...
async def run_batched_main(
    session, options_requested, date_chunk_size=100, strike_step=100  # días  # puntos
):
...
async def tasty_data(
    session, options_requested: OptionsRequest = None, equities_ticker: List[str] = []
) -> Tuple[List, List]:

    greeks_list, equities_spot = await main_downloader(
        session, equities_ticker=equities_ticker
    )
    ...
    size = round(
        size
        * (
            float(options_requested["upper_strike"])
            - float(options_requested["lower_strike"])
        )
    )

    try:
        greeks_list = await run_batched_main(
            session, options_requested, date_chunk_size=size, strike_step=size
        )
    except:
        greeks_list = await run_batched_main(
            session, options_requested, date_chunk_size=size, strike_step=size

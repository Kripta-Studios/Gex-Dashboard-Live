...
async def load(ctx, ticker: str, expiration: str, greek: str):
    ...
    if expiration.lower() not in ["0dte", "1dte", "weekly", "opex", "monthly", "all"]:
        ...
        return
    if greek.lower() not in ["delta", "gamma", "speed", "vanna", "charm", "zomma", "dgex", "vega", "vomma"]:
        await ctx.send(
            f"Invalid Greek: {greek}. Must be delta, gamma, speed, vanna, charm, zomma, dgex, vega, or vomma."
        )
        return
    await ctx.send(f"Generating plots for {ticker}/{expiration}/{greek.lower()}...")
    send = await request_plots(
        specific_ticker=ticker,
        specific_exp=expiration,
        specific_greek=greek.lower(),
        channel_id=channel_id,
    )
    if not send:
        # print(send)
        await ctx.send(
            f"Failed to send plots for {ticker}/{expiration}/{greek.lower()} sent to {channel}. Retry in a minute."
        )
    else:
        await ctx.send(
            f"Plots for {ticker}/{expiration}/{greek.lower()} sent to {channel}"
        )
